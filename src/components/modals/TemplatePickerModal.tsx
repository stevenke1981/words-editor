...
import { DEFAULT_TEMPLATE_ID, listTemplates } from '../../data/templates';
...
const TemplatePickerModal: React.FC<TemplatePickerModalProps> = ({
  ...
  const activeTemplate =
    allTemplates.find((t) => t.id === (currentTemplateId || DEFAULT_TEMPLATE_ID)) ||
    allTemplates[0];
      ...
      >
        <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50 shrink-0">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-0.5">
              {allTemplates.length}{' '}
              種寫作範例：小說、演講、書評、專訪、電子報、案例、社群、提案、兒童、社論、科幻等。套用會替換目前專案內容。
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="bg-slate-50 px-6 py-3 text-xs flex justify-between items-center border-t shrink-0">
          <span className="text-slate-500">
            目前：{activeTemplate.icon} {activeTemplate.name}
          </span>
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 text-slate-600 hover:bg-slate-200 rounded-xl"
          >
            關閉
          </button>
        </div>
