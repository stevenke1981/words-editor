      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center gap-x-2">
            ...
            <span className="text-xs bg-amber-100 text-amber-700 px-1.5 rounded-full">
              {book.goldenQuotes.length}
            </span>
          </div>
          <button
            type="button"
        ...
        <div className="p-3 max-h-[138px] overflow-auto text-sm space-y-2">
          ...
          {book.goldenQuotes.map((q) => (
            <div
              key={q.id}
              className="group bg-amber-50/60 border border-amber-100 rounded-xl p-2.5 text-xs leading-snug relative"
            >
              <div className="pr-5">「{q.text}」</div>
              <div className="mt-1 text-amber-600/70 text-[10px]">
                — {q.chapterId ? `第${q.chapterId}章` : '全書'} · {q.createdAt}
              </div>
              <button
                type="button"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center justify-between">
            <span className="section-header flex items-center gap-x-2">📑 {structureLabel}</span>
            <span className="text-xs text-slate-500">
              {book.chapters.length} {unitLabel}
            </span>
          </div>
        </div>
        ...
        <div className="p-1.5 text-sm divide-y divide-slate-100">
          {book.chapters.map((ch) => (
            <button
              ...
              className={`chapter-item w-full text-left px-3 py-2 rounded-xl cursor-pointer flex items-center justify-between gap-x-2 ${ch.id === currentChapterId ? 'active' : ''}`}
            >
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-slate-500">
                  {ch.section} · {ch.wordCount} 字
                </div>
              </div>
              <span className={`status-badge status-${ch.status} whitespace-nowrap`}>
                {ch.status}
              </span>
            </button>
          ))}
